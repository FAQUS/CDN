...
let cookiesArr = [], cookie = '', message, shareCodeList = [], shareCodeKeyList = ['pet', 'bean', 'farm', 'sgmh', 'health', 'ddfactory', 'jxfactory'];
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    let shareCode = item.assistTaskDetailVo.taskToken;
                    console.log(
                      `【京东账号${$.index}（${$.UserName}）东东工厂】${shareCode}`
                    );
                    Object.assign(shareCodeList[$.index - 1], { ddfactory: shareCode })
                  }
                });
              }
            }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  let shareCode = data.user.encryptPin;
                  console.log(`【京东账号${$.index}（${$.UserName}）京喜工厂】${shareCode}`);
                  Object.assign(shareCodeList[$.index - 1], { jxfactory: shareCode })
                }
              } else {
      ...
      try {
        if (err) {
          ...
          if (
            ...
            let shareCode = $.petInfo.shareCode
            console.log(
              `【京东账号${$.index}（${$.UserName}）京东萌宠】${shareCode}`
            );
            Object.assign(shareCodeList[$.index - 1], { pet: shareCode })
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      let shareCode = $.myPlantUuid
      console.log(`【京东账号${$.index}（${$.UserName}）种豆得豆】${shareCode}`);
      Object.assign(shareCodeList[$.index - 1], { bean: shareCode })
    } else {
      console.log(
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      let shareCode = $.farmInfo.farmUserPro.shareCode
      console.log(
        `【京东账号${$.index}（${$.UserName}）京东农场】${shareCode}`
      );
      Object.assign(shareCodeList[$.index - 1], { farm: shareCode })
    } else {
      /*console.log(
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            let shareCode = invites && invites[0]['assistTaskDetailVo']['taskToken'];
            console.log(`【京东账号${$.index}（${$.UserName}）闪购盲盒】${shareCode}`)
            Object.assign(shareCodeList[$.index - 1], { sgmh: shareCode })
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
        ...
        try {
          if (safeGet(data)) {
            ...
            if (oc(() => data.data.result.taskVos)) {
              let shareCode = oc(() => data.data.result.taskVos[0].assistTaskDetailVo.taskToken);
              console.log(`【京东账号${$.index}（${$.UserName}）京东健康】${shareCode}`);
              Object.assign(shareCodeList[$.index - 1], { health: shareCode })
            } else {
...
function getShareCodeString() {
  for (const item of shareCodeKeyList) {
    getCodeString(item)
  }
}
// 获取助力码字符串
function getCodeString(key) {
  let groupLength = 5 //切割长度
  let list = shareCodeList.map(i => i[key]).filter(i => i);
  list = list.filter(i => i);
  list = group(list, groupLength)
