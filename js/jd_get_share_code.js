                ...
                if (data.data.bizCode === 0) {
                  ...
                  $.taskVos.map((item) => {
                    if (item.taskType === 14) {
                      ...
                      dealCodeByActivity("ddfactory", `${item.assistTaskDetailVo.taskToken}`)
                    }
                  });
                }
              }
              ...
              if (safeGet(data)) {
                ...
                if (data["ret"] === 0) {
                  ...
                  if (data.factoryList && data.productionList) {
                    ...
                    dealCodeByActivity("jxfactory", `${data.user.encryptPin}`)
      ...
      try {
        if (err) {
          ...
          if (
            ...
            dealCodeByActivity("pet", `${$.petInfo.shareCode}`)
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0" &&  typeof($.plantBeanIndexResult.errorCode) == "undefined") {
      ...
      dealCodeByActivity("bean", `${$.myPlantUuid}`)
    } else {
      console.log(
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      dealCodeByActivity("farm", `${$.farmInfo.farmUserPro.shareCode}`)
    } else {
      /*console.log(
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
        ...
        // dealCodeByActivity("jxcfd", `${strMyShareId}`)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              ...
              // dealCodeByActivity("cash", `${data.data.result.inviteCode}`)
            }
          }
        }
      } catch (e) {
          ...
          try {
            if (safeGet(data)) {
              ...
              if (oc(() => data.data.result.taskVos)) {
                ...
                dealCodeByActivity("health", `${oc(() => data.data.result.taskVos[0].assistTaskDetailVo.taskToken)}`)
...
function dealCodeByActivity(key,tempValue) {
  if (activityMap.get(key) != null) {
    activityMap.get(key).push(tempValue);
  }else {
    activityMap.set(key, new Array(tempValue));
  }
